...
(function() {
  var Challenges, ready;

  Challenges = {
    init: function() {
      $('body').on('submit', '.challenge-comment-form .new_comment', this.validateChallengeComment);
      return $('body').on('click', '.search-icon', this.showSearch);
    },
    showSearch: function() {
      if ($('.search-input:visible').length < 1) {
        ...
        $('.search-input').animate({
          ...
          return $(this).css('width', 'calc(100vw - 60px)');
        });
        return $('.header-left, .mobile-header-left').animate({
          width: "0px"
        }, 300, function() {
          return $(this).hide();
        });
    ...
    validateChallengeComment: function() {
      var noSubmit;
      noSubmit = false;
      $(this).find('input, textarea').each(function() {
        if ($(this).val() === "") {
          $(this).css('border', '1px solid red');
          return noSubmit = true;
        } else {
          return $(this).attr('style', 'none');
        }
      });
      if (!$(this).find('#comment_video_url').val().match(/[-a-zA-Z0-9@:%._\+~#=]{2,256}\.[a-z]{2,6}\b([-a-zA-Z0-9@:%_\+.~#?&\/\/=]*)/)) {
        $(this).find('#comment_video_url').css('border', '1px solid red');
        noSubmit = true;
      }
...
(function() {
  ...
  Comments = {
    init: function() {
      $('body').on('ajax:success', '#main-comment-form #new_comment', this.addComment);
      $('body').on('ajax:success', '.nested-comment-form #new_comment', this.addNestedComment);
      $('body').on('click', '.comment-reply', this.showReplyForm);
      $('body').on('click', this.closeNestedCommentForm);
      $('body').on('click', '.comment-video', this.openCommetVideo);
      return $('body').on('click', '.comment-video-modal', this.closeCommentVideo);
    },
    closeCommentVideo: function(e) {
      if ($(e.target).attr('id') !== "ytplayer") {
        return $('.comment-video-modal').addClass('hidden');
      }
    },
    openCommetVideo: function() {
      return $(this).parents('.comment-block').find('.comment-video-modal').removeClass('hidden');
    },
    closeNestedCommentForm: function(e) {
      if (!$(e.target).hasClass('comment-input') && !$(e.target).hasClass('comment-reply')) {
        return $('.nested-comment-form').hide();
      }
    },
    showReplyForm: function() {
      return $(this).parents('.comment-block').find('.nested-comment-form').toggle();
    },
    addNestedComment: function(event, data) {
      var parentComment;
      parentComment = $(this).parents('.comment-block');
    ...
    addComment: function(event, data) {
      ...
      $(this)[0].reset();
      return $('.comment').first().addClass('animated fadeIn');
    },
...
(function() {
  ...
  Header = {
    init: function() {
      $('body').on('click, touchend', this.restoreHeader);
      $('body').on('click, touchend', '.menu-icon', this.showMobileDropdown);
      $('body').on('click, touchend', '.visitor-login', this.showVisitorLogin);
      $('body').on('click', '#login', this.showLoginModal);
      return $('body').on('click', this.hideLoginModal);
    },
    restoreHeader: function(e) {
      if ($(e.target).attr('id') !== 'landing-header' && $(e.target).parents('#landing-header').length < 1) {
        ...
        return $('#landing-header *').attr('style', '');
      }
    },
    showVisitorLogin: function() {
      $('.visitor-dropdown-item, .dropdown-signup').hide();
      $('#login-form').show().addClass('animated fadeIn');
      return $('.header-dropdown').animate({
        height: "190px"
      }, 300);
    },
    showMobileDropdown: function() {
      ...
      if ($('.header-dropdown:visible').length < 1) {
        ...
        return $('.visitor-dropdown-item').show().addClass('animated fadeIn');
      } else {
        $('.header-dropdown').attr('style', '').addClass('no-mobile');
        $('#landing-header').attr('style', '');
        return $('.visitor-dropdown-item').hide();
      }
    },
    showLoginModal: function() {
      return $("#login-modal").show().addClass('animated fadeInDown').find('input').first().focus();
    },
...
(function() {
  var Management, ready;

  Management = {
    init: function() {
      $('body').on('submit', '#beta-invite-form', this.popuplateUserEmailsInput);
      return $('body').on('click', '#change-password', this.showPasswordChangeContainer);
    },
    showPasswordChangeContainer: function() {
      $('#change-password-container').show();
      return $(this).hide();
    },
    popuplateUserEmailsInput: function() {
      return $('.enabled').each(function() {
        return $('#user_emails').val($(this).val() + "," + $('#user_emails').val());
      });
    }
  };

  ready = function() {
    return Management.init();
  };
...
(function() {
  ...
  Videos = {
    init: function() {
      $('body').on('click', '.modal-overlay', this.hideModal);
      $('body').on('click', '.video-play.blocked', this.showVideoModal);
      $('body').on('click', '.video-modal .btn', this.thankUser);
      $('body').on('click', '.videos-container .video-header-item', this.highlightTag);
      $('body').on('click', '.favorites-header .video-header-item', this.highlightVideoType);
      $('body').on('ajax:success', '.fav-link', this.updateLikeCount);
      $('body').on('click', '.expand-arrow', this.showExpandable);
      $('body').on('click', '#new_comment, .login-modal-trigger', this.forceLogin);
      $('body').on('click', '.show-more', this.showMore);
      $('body').on('click', '.show-less', this.showLess);
      this.clearGradients();
      if ($(window).width() > 400) {
        return this.align();
      }
    ...
    forceLogin: function() {
      console.log("here");
      return $("#login-modal").show().addClass('animated fadeInDown');
    },
    showExpandable: function() {
      return $(this).parent().find('.expandable').toggle().addClass('animated fadeIn');
    },
    clearGradients: function() {
      return $('.series-content').each(function() {
        var videoCount;
        videoCount = $(this).find('.series-video').length;
        if (videoCount < 5) {
          return $(this).find('.white-gradient, .scroll-arrow').hide();
        }
      });
    },
    updateLikeCount: function(event, data) {
      $(this).parent().find('.video-like-count').text(data.like_count);
      $(this).toggleClass('selected');
      if ($(this).parents('.video-main-info').length > 0) {
        $('.video-item.current .fav-link').toggleClass('selected');
      }
      if ($(this).parents('.video-item.current').length > 0) {
        return $('.video-main-info .video-like-count').text(data.like_count);
      }
    },
    highlightVideoType: function() {
      $('.favorited-content').hide();
      $('.video-header-item').removeClass('active');
      $(this).addClass('active');
      return $('.' + $(this).attr('data-show')).show();
    },
    align: function() {
      $('.video-item-container').css('margin-right', '10px');
      return $('.regular-videos .video-item-container:visible').each(function(i) {
        if ((i + 1) % 5 === 0) {
          return $(this).css('margin-right', '0px');
        }
      });
    },
    highlightTag: function() {
      ...
      return Videos.filterVideos();
    },
    filterVideos: function() {
      var filters;
      filters = $('.video-header-item.active p').map(function(i, v) {
        return v.className;
      });
      filters = $.makeArray(filters).join("+");
      return $.get("/videos?tags=" + filters, function(data) {
    ...
    showVideoModal: function(e) {
      if (!$(e.target).hasClass('modal-overlay')) {
        return $(this).parent().find('.modal-overlay').show();
      }
    },
    thankUser: function() {
      $(this).parents('.video-modal').find('.modal-message').text("Thanks for your interest, we'll let you know it's available.");
      return $(this).hide();
    }
  };
...
(function() {
  ...
  Visitors = {
    init: function() {
      ...
      if (!this.mobile) {
        $('.white-gradient.right').on('mouseenter', this.scrollVideosRight);
      }
      $('.white-gradient.left').on('mouseenter', this.scrollVideosLeft);
      if (!this.mobile) {
        $('.series-videos').scroll(this.showScrollBack);
      }
      $('.white-gradient').on('mouseleave', this.stopScrollVideos);
      this.scrollSpeed = 2;
      $('.vbox').removeClass('vbox');
      $('body').on('ajax:success', '.waitlist-form', this.thankUser);
      $('body').on('submit', '.waitlist-form', this.checkForm);
      $('body').on('submit', '#invite-accept-form:not(.user-edit)', this.checkInviteAcceptForm);
      $('body').on('submit', '#login-form', this.checkLoginForm);
      $('body').on('keydown', '.invite-email', this.verifyEmail);
      if ($('.invite-email').length > 0 && $('.invite-email').val().match(/.+\@.+\..+/)) {
        $('.email-check').show();
      }
      return $('body').on('click', '.welcome-close', this.closeWelcome);
    },
    showScrollBack: function() {
      if ($(this).scrollLeft() > 5) {
        $(this).parents('.series-content').find('.white-gradient.left').show();
        return $(this).parents('.series-content').find('.scroll-arrow.left').show();
      } else {
        $(this).parents('.series-content').find('.white-gradient.left').hide();
        return $(this).parents('.series-content').find('.scroll-arrow.left').hide();
      }
    },
    closeWelcome: function() {
      $('#welcome-container').hide();
      return history.pushState("home", "home", "/");
    },
    verifyEmail: function() {
      if ($('.invite-email').val().match(/.+\@.+\..+/)) {
        return $('.email-check').show();
      } else {
        return $('.email-check').hide();
      }
    },
    checkInviteAcceptForm: function() {
      var noEmail, noName, noPassword, noPasswordConf;
      noName = $('.invite-name').val() === "";
      noEmail = $('.invite-email').val() === "";
      noPasswordConf = $('.invite-password-conf').val() === "";
      noPassword = $('.invite-password').val() === "";
      if (noName || noEmail || noPasswordConf || noPassword) {
        if (noName) {
          $('.invite-name').css('border', '1px solid #e1474c');
        }
        if (noEmail) {
          $('.invite-email').css('border', '1px solid #e1474c');
        }
        if (noPassword) {
    ...
    checkLoginForm: function() {
      var noEmail, noPassword;
      noEmail = $(this).find('.login-email').val() === "";
      noPassword = $(this).find('.login-password').val() === "";
      if (noEmail || noPassword) {
        if (noPassword) {
          $('.login-password').css('border', '1px solid #e1474c');
        }
        if (noEmail) {
          $('.login-email').css('border', '1px solid #e1474c');
        }
    ...
    checkForm: function() {
      ...
      if (noName || noEmail) {
        if (noName) {
          $('#name').css('border', '1px solid #e1474c');
        }
        if (noEmail) {
          $('#email').css('border', '1px solid #e1474c');
        }
        return false;
      }
    },
    thankUser: function() {
      ...
      return $('.waitlist-form')[0].reset();
    },
    scrollVideosRight: function() {
      var videoList;
      videoList = $(this).parent().find('.series-videos');
      $(this).parent().find('.white-gradient.left, .scroll-arrow.left').removeClass('hidden');
      clearInterval(Visitors.scrollIntervalId);
      return Visitors.scrollIntervalId = setInterval(function() {
        var pos;
        pos = videoList.scrollLeft();
        return videoList.scrollLeft(pos + Visitors.scrollSpeed);
      }, 5);
    },
    scrollVideosLeft: function() {
      var div, gradient;
      gradient = $(this);
      div = $(this).parent().find('.series-videos');
      clearInterval(Visitors.scrollIntervalId);
      return Visitors.scrollIntervalId = setInterval(function() {
        var pos;
        pos = div.scrollLeft();
        if (div.scrollLeft() < 1) {
          gradient.parent().find('.white-gradient.left, .scroll-arrow.left').addClass('hidden');
        }
        return div.scrollLeft(pos - Visitors.scrollSpeed);
      }, 5);
    },
    stopScrollVideos: function(e) {
      if (!$(e.relatedTarget).hasClass('scroll-arrow')) {
        return clearInterval(Visitors.scrollIntervalId);
      }
    }
  };

  ready = function() {
    return Visitors.init();
  };
